...
public class Aplicacion11_24 {
	...
	public static void main(String[] args) {
		...
		fusionarBase(fich1, fich2);
		try (ObjectInputStream in = new ObjectInputStream(new FileInputStream(fich1))) {
			tabla3=(Integer[]) in.readObject();
		}catch(IOException|ClassNotFoundException ex) {
			System.out.println(ex.getMessage());
		}
		System.out.println(Arrays.toString(tabla3));
	}
	public static void fusionarBase(String ficheroBase, String ficheroNuevo) {
		Integer []fusion=new Integer[10];
		fusion=fusionar(ficheroBase,ficheroNuevo);
		try (ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream(ficheroBase))) {
			out.writeObject(fusion);
		}catch(IOException ex) {
			System.out.println(ex.getMessage());
		}
	}
